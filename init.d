...
NMBDPID=/var/samba/nmbd.pid
SMBDPID=/var/samba/smbd.pid
...
case "$1" in
	...
	stop)
		...
		echo -n " nmbd"
		start-stop-daemon --stop --quiet --pidfile $NMBDPID --exec /usr/sbin/nmbd -- -D

		echo -n " smbd"
		start-stop-daemon --stop --quiet --pidfile $SMBDPID --exec /usr/sbin/smbd -- -D

		echo "."
		;;
	reload)
		echo -n "Reloading /etc/samba/smb.conf (smbd only)"
		start-stop-daemon --stop --signal HUP --pidfile /var/samba/smbd.pid --exec /usr/sbin/smbd -- -D
