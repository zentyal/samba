...
NTSTATUS dcerpc_pipe_open_roh_recv(
		struct tevent_req *req,
		TALLOC_CTX *mem_ctx,
		struct tstream_context **stream,
		...
		struct resolve_context *resolve_ctx)
{
	...
	req = tevent_req_create(conn, &state,
			struct roh_open_connection_state);
	if (req == NULL) {
		return NULL;
	}
...
static void roh_continue_resolve_name(struct composite_context *ctx)
{
	 ...
	 */
	state->roh->connection_state = ROH_STATE_OPEN_START;
	subreq = roh_connect_channel_in_send(state, state->event_ctx,
		state->rpcproxy_addresses[state->rpcproxy_address_index],
		state->rpc_proxy_port, state->credentials,
		state->roh,
		state->tls, state->tls_params);
	if (tevent_req_nomem(subreq, state->req)) {
		return;
	}
	tevent_req_set_callback(subreq, roh_connect_channel_in_done,
			state->req);
}
...
static void roh_connect_channel_in_done(struct tevent_req *subreq)
{
	...
	subreq = roh_connect_channel_out_send(state, state->event_ctx,
		state->rpcproxy_addresses[state->rpcproxy_address_index],
		state->rpc_proxy_port, state->credentials,
		state->roh, state->tls, state->tls_params);
	if (tevent_req_nomem(subreq, state->req)) {
		return;
	}
	tevent_req_set_callback(subreq, roh_connect_channel_out_done,
			state->req);
}
...
static void roh_connect_channel_out_done(struct tevent_req *subreq)
{
	...
	subreq = roh_send_RPC_DATA_IN_send(state,
			state->event_ctx,
			state->credentials,
			state->roh,
			state->rpc_server,
...
static void roh_send_RPC_DATA_OUT_done(struct tevent_req *subreq)
{
	...
	subreq = roh_send_CONN_A1_send(state, state->event_ctx,
			state->roh);
...
static void roh_send_CONN_A1_done(struct tevent_req *subreq)
{
	...
	subreq = roh_send_CONN_B1_send(state, state->event_ctx,
			state->roh);
...
static void roh_send_CONN_B1_done(struct tevent_req *subreq)
{
	...
	subreq = roh_recv_out_channel_response_send(state, state->event_ctx,
			state->roh);
	tevent_req_set_callback(subreq, roh_recv_out_channel_response_done,
			req);
}
...
static void roh_recv_out_channel_response_done(struct tevent_req *subreq)
{
	...
	subreq = roh_recv_CONN_A3_send(state, state->event_ctx,
			state->roh);
...
static void roh_recv_CONN_A3_done(struct tevent_req *subreq)
{
	...
	subreq = roh_recv_CONN_C2_send(state, state->event_ctx,
			state->roh);
...
static void tstream_roh_readv_handler(struct tevent_req *subreq)
{
	struct tevent_req *req = tevent_req_callback_data(subreq,
			struct tevent_req);
	struct tstream_roh_readv_state *state = tevent_req_data(req,
			struct tstream_roh_readv_state);
	int ret;
	int sys_errno;
...
static void tstream_roh_writev_handler(struct tevent_req *subreq)
{
	struct tevent_req *req = tevent_req_callback_data(subreq,
			struct tevent_req);
	struct tstream_roh_writev_state *state = tevent_req_data(req,
			struct tstream_roh_writev_state);
	int nwritten;
	int sys_errno;
...
static void tstream_roh_disconnect_channel_in_handler(
		struct tevent_req *subreq);
		...
		struct tstream_context *stream)
{
	...
	subreq = tstream_disconnect_send(state, ev,
			ctx->roh_conn->default_channel_in->streams.active);
	if (tevent_req_nomem(subreq, req)) {
		goto post;
	}
	tevent_req_set_callback(subreq,
			tstream_roh_disconnect_channel_in_handler, req);
...
static void tstream_roh_disconnect_channel_out_handler(
		struct tevent_req *subreq);
		...
		struct tevent_req *subreq)
{
	struct tevent_req *req = tevent_req_callback_data(subreq,
			struct tevent_req);
	struct tstream_roh_disconnect_state *state = tevent_req_data(req,
			struct tstream_roh_disconnect_state);
	struct tstream_context *stream = state->stream;
	struct tstream_roh_context *roh_stream =
		tstream_context_data(stream, struct tstream_roh_context);
	int ret;
	int sys_errno;
	...
	subreq = tstream_disconnect_send(state, state->ev,
		roh_stream->roh_conn->default_channel_out->streams.raw);
	tevent_req_set_callback(subreq,
			tstream_roh_disconnect_channel_out_handler, req);
