		...
		struct tstream_tls_params *tls_params)
{
	...
	DEBUG(8, ("%s: Connecting channel in socket, "
	...
	if (ret != 0) {
		DEBUG(0, ("%s: Cannot create remote socket address, "
				"error: %s (%d)\n",
				__func__, strerror(errno), errno));
		status = map_nt_error_from_unix_common(errno);
		tevent_req_nterror(req, status);
...
static void roh_connect_channel_in_done(struct tevent_req *subreq)
{
	...
	if (state->tls) {
		...
		tevent_req_set_callback(subreq,
				roh_connect_channel_in_tls_done, req);
		return;
	}
		...
		const char *rpc_proxy)
{
	...
	subreq = http_send_request_send(mem_ctx, ev,
			...
			roh->default_channel_in->send_queue, state->request);
...
static void roh_send_RPC_DATA_IN_done(struct tevent_req *subreq)
{
	...
	DEBUG(8, ("%s: RPC_IN_DATA sent (%d bytes written)\n",
