		...
		struct tstream_tls_params *tls_params)
{
	...
	DEBUG(8, ("%s: Connecting channel out socket, "
	...
	ret = tsocket_address_inet_from_strings(state, "ipv4", NULL, 0,
			&state->local_address);
	if (ret != 0) {
		DEBUG(0, ("%s: Cannot create local socket address, "
				"error: %s (%d)\n", __func__,
	...
	if (ret != 0) {
		DEBUG(1, ("%s: Cannot create remote socket address, "
				"error: %s (%d)\n", __func__,
				strerror(errno), errno));
		status = map_nt_error_from_unix_common(errno);
		tevent_req_nterror(req, status);
...
static void roh_connect_channel_out_done(struct tevent_req *subreq)
{
	...
	if (state->tls) {
		...
		tevent_req_set_callback(subreq,
				roh_connect_channel_out_tls_done, req);
		return;
	}
		...
		const char *rpc_proxy)
{
	...
	/* Create the HTTP channel OUT request as specified in
	 * section 2.1.2.1.2 */
	...
	creds = talloc_asprintf(state, "%s:%s", credentials->username,
	...
	subreq = http_send_request_send(mem_ctx, ev,
			...
			roh->default_channel_out->send_queue, state->request);
...
static void roh_send_RPC_DATA_OUT_done(struct tevent_req *subreq)
{
	...
	DEBUG(8, ("%s: RPC_OUT_DATA sent (%d bytes written)\n",
...
static void roh_send_CONN_A1_done(struct tevent_req *subreq)
{
	...
	DEBUG(8, ("%s: CONN/A1 sent (%d bytes)\n",
		...
		struct roh_connection *roh)
{
	...
	tevent_req_set_callback(subreq,
			roh_recv_out_channel_response_done, req);
...
static void roh_recv_out_channel_response_done(struct tevent_req *subreq)
{
	...
	state->bytes_readed = http_read_response_recv(subreq, state,
			&state->response, &sys_errno);
	...
	DEBUG(8, ("%s: RCP_OUT_DATA Response received (%d bytes read)\n",
	...
	switch (code) {
		...
		case 401:
			...
			return;
		case 503:
			/* TODO Decode error info as specified in
			 * section 2.1.2.1.3 */
...
static void roh_recv_CONN_C2_done(struct tevent_req *subreq)
{
	...
	state->recv_window_size =
		rts.Commands[1].Command.ReceiveWindowSize.ReceiveWindowSize;
	state->connection_timeout =
		rts.Commands[2].Command.ConnectionTimeout.ConnectionTimeout;

	DEBUG(8, ("%s: CONN/C2 received, version is %u, "
		"receive windows size is %u, connection timeout is %u\n",
