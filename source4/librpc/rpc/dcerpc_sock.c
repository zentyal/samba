...
/*
  ...
  on IP transport.
*/
static void continue_http_open_socket(struct composite_context *ctx)
{
	struct composite_context *c = talloc_get_type(ctx->async.private_data,
						      struct composite_context);
	...
	if (!NT_STATUS_IS_OK(c->status)) {
		...
		if (s->addresses[s->index]) {
			...
			sock_http_req = dcerpc_pipe_open_socket_send(c, s->conn, s->localaddr,
								s->srvaddr, s->target_hostname,
								NULL,
								NCACN_HTTP);
			composite_continue(c, sock_http_req, continue_http_open_socket, c);
			return;
		} else {
			composite_error(c, c->status);
			return;
